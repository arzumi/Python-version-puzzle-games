...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self) -> bool:
        ...
        for r in range(len(self._grid)):
            for c in range(len(self._grid[0])):
                result = self._fail_test_helper(r, c)
                if result:
                    return result

        return False

    def _fail_test_helper(self, row: int, col: int) -> Optional[bool]:
        """
        This is a private helper method that helps with checking the conditions
        inside the sudoku puzzle.
        """
        if self._grid[row][col] == EMPTY_CELL:
            allowed_symbols = (self._symbol_set
                               - (self._row_set(row)
                                  | self._column_set(col)
                                  | self._subsquare_set(row, col)))
    ...
    def has_unique_solution(self) -> bool:
        ...
        """
        solver = DfsSolver()
        solution1 = solver.solve(self)
        if not solution1:
            return False
        else:
            str_rep = set()
            for item in solution1:
                str_rep.add(str(item))
            solution2 = solver.solve(self, str_rep)
            if len(solution2) == 0:
                return True
            else:
                return False
