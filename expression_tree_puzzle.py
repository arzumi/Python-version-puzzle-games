...
class ExpressionTreePuzzle(Puzzle):
    ...
    def extensions(self) -> List[ExpressionTreePuzzle]:
        ...
        for variable in self.variables:
            ...
            if value == 0:
                for i in range(1, 10):
                    possible_extention = ExpressionTreePuzzle(self._tree,
                                                              self.target)
                    possible_extention.variables = self.variables.copy()
                    possible_extention.variables[variable] = i

                    result.append(possible_extention)
    ...
    def fail_fast(self) -> bool:
        ...
        """
        upper_subs = self.variables.copy()
        for key in upper_subs:
            if upper_subs[key] == 0:
                upper_subs[key] = 9
        upper_temp = self._tree.copy()
        upper_est = upper_temp.eval(upper_subs)

        lower_subs = self.variables.copy()
        lower_temp = self._tree.copy()
        lower_est = lower_temp.eval(lower_subs)

        return not upper_est >= self.target >= lower_est
