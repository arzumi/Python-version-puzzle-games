...
class WordLadderPuzzle(Puzzle):
    ...
    def extensions(self) -> List[WordLadderPuzzle]:
        ...
        >>> wl3 = WordLadderPuzzle("same", "cost")
        >>> len(wl3.extensions()) > 0
        True
        """
        result = []
        for i in range(len(self.from_word)):
            for j in range(len(LETTERS)):
                temp = self.from_word[:i] + LETTERS[j] + self.from_word[i + 1:]
                if temp in self.word_set and temp != self.from_word:
                    result.append(WordLadderPuzzle(temp,
                                                   self.to_word, self.word_set))
        return result

        # result = []
        # for item in self.word_set:
        #     count = 0
        #     for i in range(len(item)):
        #         if len(item) == len(self.from_word) and \
        #                 item[i] != self.from_word[i]:
        #             count += 1
        #     if count == 1:
        #         result.append(WordLadderPuzzle(item, self.to_word,
        #                                        self.word_set))
        # return result
    ...
    def get_difficulty(self) -> str:
        ...
        """
        chosen_solver = BfsSolver()
        result = chosen_solver.solve(self)
