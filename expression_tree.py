...
class ExprTree:
    ...
    def eval(self, lookup: Dict[str, int]) -> int:
        ...
        if isinstance(self._root, int):
            ...
            result = self._subtrees[0].eval(lookup)
...
def construct_from_list(values: List[List[Union[str, int]]]) -> ExprTree:
    ...
    >>> s = [['+'], [3, '*', 'a', '+'], ['a', 'b', '+'], [5, 'c'], ['a', 'b']]
    >>> actual = construct_from_list(s)
    >>> print(actual)
    (3 + (a * b * (a + b)) + a + (5 + c))
    """

    main_root = values[0][0]
    first_branch = ExprTree(main_root, [])
    if main_root not in OPERATORS:
        return first_branch

    op_queue = Queue()
    op_queue.enqueue(first_branch)

    for i in range(1, len(values)):
        curr_op = op_queue.dequeue()
        for item in values[i]:
            temp = ExprTree(item, [])
            curr_op.append(temp)
            if item in OPERATORS:
                op_queue.enqueue(temp)
